...
function resolveConnectTimeout() {
    ...
    if (!Number.isFinite(parsedTimeout) || parsedTimeout <= 0) {
        console.error('DISCORD_CONNECT_TIMEOUT_MS must be a positive integer representing milliseconds. '
            + `Received: ${rawTimeout}`);
        process.exit(1);
    }
    return parsedTimeout;
}
function createDiscordDispatcher() {
...
async function deploy() {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            console.log(`Deploying ${guildCommands.length} guild commands and ${globalCommands.length} global commands... `
            ...
            if (guildId) {
                ...
                console.warn('Skipping guild-only commands because DISCORD_GUILD_ID is not set: '
                    + guildOnlyNames.join(', '));
            }
