...
import type { BotClient } from './types/bot-client.js';
...
client.commands = new Map<string, CommandModule>();

let clientReadyAt: Date | null = null;
let commandsLoaded = 0;
const recentEvents: RecentEvent[] = [];
const MAX_EVENTS = 25;
const commandExtensions = new Set(['.js', '.ts']);

function pushEvent(type: string, detail: string, level: EventLevel = 'info') {
...
async function importCommandFresh(
  filePath: string,
  categoryName: string,
  cacheBustValue: number
): Promise<CommandModule | undefined> {
  const fileUrl = url.pathToFileURL(filePath);
  const freshUrl = new URL(fileUrl.href);
  freshUrl.searchParams.set('update', String(cacheBustValue));

  try {
    const commandModule = (await import(freshUrl.href)) as { default?: CommandModule };
    return (commandModule?.default ?? commandModule) as CommandModule | undefined;
  } catch (error) {
    const err = error as Error;
    console.error(`Failed to load command module ${categoryName}/${path.basename(filePath)}`, err);
    pushEvent(
      'Command Error',
      `Failed to load ${categoryName}/${path.basename(filePath)}: ${err.message ?? 'Unknown error.'}`,
      'error'
    );
    return undefined;
  }
  ...
  for (const category of categories) {
    ...
    for (const file of files) {
      ...
      if (!command?.data?.name || typeof command.execute !== 'function') {
        ...
        pushEvent(
          'Command Warning',
          `Skipped invalid command ${category.name}/${file}.`,
          'warning'
        );
        continue;
      }
...
async function reloadCommands(reasonForEvent?: string): Promise<ReloadResult> {
...
function startAutoReload() {
  ...
  pushEvent(
    'Commands',
    `Scheduled automatic reload every ${Math.round(COMMAND_AUTO_RELOAD_MS / 60000)} minutes.`,
    'info'
  );
}
...
function getShardLatencies(): number[] {
  return Array.from(client.shards.values())
    .map((shard) => shard.latency)
    .filter((latency) => Number.isFinite(latency) && latency > 0);
}

function getAverageShardLatency(): number | null {
...
function getLastHeartbeatTimestamp(): number | null {
