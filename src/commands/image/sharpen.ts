...
const SHARPEN_KERNELS = {
  ...
  'light' | 'medium' | 'strong',
  { label: string; matrix: number[][]; message: string }
>;

type SharpenStrength = keyof typeof SHARPEN_KERNELS;
...
const command: CommandModule = {
  ...
  async execute(interaction) {
    const strength = (interaction.options.getString('strength') as SharpenStrength | null) ?? 'medium';
