...
const command: CommandModule = {
  ...
  async execute(interaction) {
    ...
    if (!interaction.inGuild() || !channel || typeof (channel as { edit?: unknown }).edit !== 'function') {
      await interaction.editReply('This command can only be used in a server text channel.');
      return;
    }

    try {
      await (channel as { edit: (options: { rateLimitPerUser: number; reason: string }) => Promise<unknown> }).edit({
